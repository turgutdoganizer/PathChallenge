...
using PathProjectChallenge.Data.DataProviders.LinqToDb;
using System;
using System.Data;
...
namespace PathProjectChallenge.Data.DataProviders
{
    public partial class PostgreSqlDataProvider : BaseDataProvider, IPathDataProvider
    {
        #region Fields
        ...
        private string GetSequenceName<TEntity>(DataConnection dataConnection) where TEntity : BaseEntity
        {
            ...
            if (descriptor is null)
                throw new PathException($"Mapped entity descriptor is not found: {typeof(TEntity).Name}");
            ...
            if (string.IsNullOrEmpty(columnName))
                throw new PathException("A table's primary key does not have an identity constraint");
        ...
        public virtual string BuildConnectionString(IPathConnectionStringInfo nopConnectionString)
        {
            if (nopConnectionString is null)
                throw new ArgumentNullException(nameof(nopConnectionString));

            if (nopConnectionString.IntegratedSecurity)
                throw new PathException("Data provider supports connection only with login and password");
